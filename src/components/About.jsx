...
export default function About(){
    const steps = [
      ...
      icon: <Users className="w-5 h-5 text-[#FFDE59]" />,
    },
    {
      num: "02",
      ...
      icon: <LayoutGrid className="w-5 h-5 text-[#FFDE59]" />,
    },
    {
      num: "03",
      ...
      icon: <Video className="w-5 h-5 text-[#FFDE59]" />,
    },
    {
      num: "04",
      ...
      icon: <Award className="w-5 h-5 text-[#FFDE59]" />,
    },
    ];
    return (
        <section id="#about" className="flex flex-col items-center gap-10   max-w-7xl mx-auto">
          ...
          <div className="text">
            <p className=" text-zinc-400 max-w-3xl font-light text-center  leading-relaxed text-base">Brilliant Move is a <span className="text-[#ffde59] font-semibold">5-week 
              summer hackathon</span> for students aged 14–18. Teams of up to 4 build real projects 
              across 4 tracks, document their journey through a  <span className="text-[#ffde59] font-semibold">very short film</span>, and compete in a 
              festival-style closing ceremony 
               all while learning from expert workshops every week.</p>
          </div>
          <div className="inline-flex items-center gap-3 bg-[#FFDE59]/8 border border-[#FFDE59]/25 rounded-full px-5 py-2.5 mb-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 lg:gap-8 items-stretch">
          {steps.map((step, i) => (
            <div key={step.num} className="relative w-full h-full">
              <div className="group flex flex-col items-center text-center border border-white/8 bg-white/3 rounded-2xl p-7 transition-all duration-300 hover:border-[#FFDE59]/40 hover:bg-[#FFDE59]/4 hover:-translate-y-1 cursor-default w-full h-full">
                <div className="text-[0.7rem] font-header font-black uppercase tracking-widest text-white/20 group-hover:text-[#FFDE59] transition-colors duration-300 mb-4">
                  Step {step.num}
                </div>

                <div className="w-12 h-12 shrink-0 rounded-xl bg-white/6 border border-white/10 group-hover:bg-[#FFDE59]/10 group-hover:border-[#FFDE59]/50 transition-all duration-300 flex items-center justify-center mb-4">
                  {step.icon}
                </div>
